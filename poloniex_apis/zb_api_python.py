...
from api_key_secret_util import get_chbtc_api_key, get_chbtc_api_secret, get_zb_bch_address
...
class zb_api:
    ...
    def sell_eth_order(self, price, amount):
        try:
            params = "method=order&accesskey=" + self.mykey + "&price=" + str(price) + "&amount=" + str(amount) + "&tradeType=0&currency=eth_cny"
            path = 'order'
    ...
    def sell_bcc_order(self, price, amount):
        try:
            params = "method=order&accesskey=" + self.mykey + "&price=" + str(price) + "&amount=" + str(amount) + "&tradeType=0&currency=bcc_usdt"
            path = 'order'
    ...
    def buy_eth_order(self, price, amount):
        try:
            params = "method=order&accesskey=" + self.mykey + "&price=" + str(price) + "&amount=" + str(amount) + "&tradeType=1&currency=eth_cny"
            path = 'order'
    ...
    def buy_bcc_order(self, price, amount):
        try:
            params = "method=order&accesskey=" + self.mykey + "&price=" + str(price) + "&amount=" + str(amount) + "&tradeType=1&currency=bcc_usdt"
            path = 'order'
    ...
    def withdraw(self, currency, amount, address):
        try:
            params = 'accesskey=' + self.mykey +'&amount='+ amount+ '&currency=' + currency + '&fees=0.005&itransfer=0&method=withdraw'+ '&receiveAddr=' +address+'&safePwd=zsj,123456'
            path = 'withdraw'
            obj = self.__api_call(path, params)

            if obj["code"] != 1000:
                logger.error('withdraw : %s : %s : to %s : %s : %s', amount, currency, address,  obj["code"], obj["message"])
                return "error"
            return obj
    ...
    def get_zb_withdraw_status(self, currency):
        try:
            params = "accesskey=" + self.mykey + "&currency="+currency+"&method=getWithdrawRecord&pageIndex=1&pageSize=10"
            path = 'getWithdrawRecord'
            obj = self.__api_call(path, params)
            return obj
    ...
    def query_market(self, currency):
        try:
            url = "http://api.zb.com/data/v1/ticker?market="+currency
            request = urllib2.Request(url)
    ...
    def query_depth(self, currency):
        try:
            url = "http://api.zb.com/data/v1/depth?market="+currency+"&size=10"
            request = urllib2.Request(url)
            response = urllib2.urlopen(request, timeout=10)
            obj = json.loads(response.read())
            return obj
